...
func (s *FeatureFlagService) GetFeatureFlags(organizationID uuid.UUID) ([]types.FeatureFlag, error) {
	...
	if len(flags) == 0 {
		...
		defaultFeatures := []types.FeatureName{types.FeatureDomain, types.FeatureTerminal, types.FeatureNotifications, types.FeatureFileManager, types.FeatureSelfHosted, types.FeatureAudit, types.FeatureGithubConnector, types.FeatureMonitoring, types.FeatureContainer, types.FeatureMCP}
