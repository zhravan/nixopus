...
export const COMMANDS = {
  curl: 'curl -sSL https://install.nixopus.com | bash',
...
export const DEFAULT_FEATURES: FeatureOption[] = [
  // Network
  createFeature('apiDomain', 'API Domain', 'HTTPS endpoint for API', 'network', {
    inputType: 'text',
    placeholder: 'api.example.com',
    tooltip: 'Domain for API server. Requires App Domain for auto HTTPS with Let\'s Encrypt.',
  }),
  createFeature('viewDomain', 'App Domain', 'HTTPS endpoint for dashboard', 'network', {
    inputType: 'text',
    placeholder: 'app.example.com',
    tooltip: 'Domain for dashboard. Requires API Domain for auto HTTPS with Let\'s Encrypt.',
  }),
  createFeature('hostIp', 'Host IP', 'Server IP (auto-detected if empty)', 'network', {
    inputType: 'text',
  ...
  // Database
  createFeature('externalDb', 'External Database', 'Use external PostgreSQL', 'database', {
    inputType: 'text',
    placeholder: 'postgresql://user:pass@host:5432/db',
    tooltip: 'Use existing PostgreSQL instead of local DB. Ignores DB/Redis port settings.',
  }),
  ...
  createFeature('caddyAdminPort', 'Caddy Admin Port', 'Default: 2019', 'ports', {
    inputType: 'number',
    placeholder: '2019',
    tooltip: 'Port for Caddy admin API. Used for proxy configuration management.',
  }),
  createFeature('caddyHttpPort', 'Caddy HTTP Port', 'Default: 80', 'ports', {
    inputType: 'number',
    placeholder: '80',
    tooltip: 'Port for Caddy HTTP traffic. Ports < 1024 need root/sudo privileges.',
  }),
  createFeature('caddyHttpsPort', 'Caddy HTTPS Port', 'Default: 443', 'ports', {
    inputType: 'number',
    placeholder: '443',
    tooltip: 'Port for Caddy HTTPS traffic. Ports < 1024 need root/sudo privileges.',
  }),
  createFeature('supertokensPort', 'SuperTokens Port', 'Default: 3567', 'ports', {
    inputType: 'number',
    placeholder: '3567',
  ...
  createFeature('configFile', 'Config File', 'Custom config file path', 'options', {
    inputType: 'text',
    placeholder: '/path/to/config.yaml',
    tooltip: 'Path to custom config file. Defaults to built-in config if not provided.',
  }),
  createFeature('noRollback', 'No Rollback', 'Disable automatic rollback', 'options', {
    inputType: 'toggle',
  ...
  createFeature('healthCheckTimeout', 'Health Check Timeout', 'Default: 120 seconds', 'options', {
    inputType: 'number',
    placeholder: '120',
    tooltip: 'Maximum time to wait for services to become healthy (in seconds).',
  }),
