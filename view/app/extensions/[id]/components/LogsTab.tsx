...
import { useGetExecutionLogsQuery, useListExecutionsQuery } from '@/redux/services/extensions/extensionsApi';
    ...
    if (logsResp.logs && logsResp.logs.length > 0) {
      ...
      setLines((prev) => {
        const appended = logsResp.logs.map((l) => {
          ...
          const dataStr = l.data ? ` ${typeof l.data === 'string' ? l.data : JSON.stringify(l.data)}` : '';
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="sm:max-w-2xl">
          ...
          <div className="p-3">
            ...
            <AceEditor
              onLoad={(editor: any) => { editorRef.current = editor; }}
              mode="sh"
              value={lines.join('\n')}
              onChange={() => {}}
              name="extension-logs"
