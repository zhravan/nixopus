...
export default function ExtensionInput({ open, onOpenChange, extension, onSubmit }: ExtensionInputProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg sm:max-w-xl">
        ...
        <div className="space-y-4 py-2">
          {variables.length === 0 && <div className="text-sm text-muted-foreground">{t('extensions.noVariables')}</div>}
          {variables.map((v) => (
            <div key={v.id} className="space-y-1">
              <Field variable={v} value={values[v.variable_name]} onChange={handleChange} />
              {errors[v.variable_name] && <div className="text-xs text-destructive">{errors[v.variable_name]}</div>}
            </div>
          ))}
        </div>
...
function Field({ variable, value, onChange }: { variable: ExtensionVariable; value: unknown; onChange: (name: string, value: unknown) => void }) {
  const id = `var-${variable.variable_name}`;
  if (variable.variable_type === 'boolean') {
    return (
      <div className="flex items-center gap-3">
        <Checkbox id={id} checked={Boolean(value)} onCheckedChange={(v) => onChange(variable.variable_name, Boolean(v))} />
  ...
  if (variable.variable_type === 'array') {
    const textValue = Array.isArray(value) ? (value as unknown[]).map((v) => String(v)).join('\n') : String(value ?? '');
    return (
      <div className="grid gap-1">
        <Label htmlFor={id}>{variable.variable_name}</Label>
        <textarea
          ...
          value={textValue}
          onChange={(e) => onChange(variable.variable_name, e.target.value.split('\n').map((s) => s.trim()).filter((s) => s.length > 0))}
  ...
  return (
    <div className="grid gap-1">
      ...
      <Input
        ...
        onChange={(e) => onChange(variable.variable_name, variable.variable_type === 'integer' ? Number(e.target.value) : e.target.value)}
        placeholder={variable.description || variable.variable_name}
      />
    </div>
  );
