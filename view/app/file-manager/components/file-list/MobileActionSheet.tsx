...
export const MobileActionSheet: React.FC<MobileActionSheetProps> = ({ 
  ...
  const actions = [
    ...
    ...(type === 'file' ? [{ id: 'download', label: t('fileManager.contextMenu.download'), icon: Download, onClick: () => {} }] : []),
    { id: 'delete', label: t('fileManager.contextMenu.delete'), icon: TrashIcon, onClick: onDelete, destructive: true },
  ];
        ...
        <SheetHeader className="pb-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0 text-left">
              ...
              <SheetDescription>
                {file.file_type === FileType.File ? formatFileSize(file.size) : 'Folder'} • {new Date(file.updated_at).toLocaleDateString()}
              </SheetDescription>
            </div>
          </div>
        </SheetHeader>
